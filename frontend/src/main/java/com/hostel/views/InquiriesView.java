...
public class InquiriesView extends VBox {
    ...
    public InquiriesView() {
        ...
        Label title = new Label("Inquiries");
        ...
        addColumn("Description", "description", 200);
        addColumn("Start Date", "startDate", 120);
        addColumn("Status", "status", 100);
        addColumn("Officer", "officerName", 140);
    ...
    private void loadData() {
        new Thread(() -> {
            try {
                JsonNode response = ApiClient.get("/inquiries");
                Platform.runLater(() -> {
                    ObservableList<JsonNode> items = FXCollections.observableArrayList();
                    JsonNode data = response.has("inquiries") ? response.get("inquiries") : response;
                    if (data.isArray()) for (JsonNode node : data) items.add(node);
                    table.setItems(items);
                    table.setVisible(true);
                    statusLabel.setText(items.size() + " inquiries loaded");
                });
            } catch (Exception ex) {
