...
export default function Roommates() {
  ...
  const [form, setForm] = useState({ sleepSchedule: 'flexible', studyHabit: 'moderate', neatnessLevel: 'moderate' });
  ...
  useEffect(() => {
    if (pref) {
      setForm({
        sleepSchedule: pref.sleepSchedule || 'flexible',
    ...
    <div className="fade-in">
      ...
      {isStudent && pref && (
        <div className="info-box mb-24">
          ...
          Neatness: <em>{pref.neatnessLevel?.replace('_', ' ')}</em>
        </div>
      )}
      ...
      {showForm && (
        <Modal title="Set Roommate Preferences" onClose={() => setShowForm(false)}>
          ...
          <select className="apple-input" value={form.studyHabit} onChange={e => setForm({ ...form, studyHabit: e.target.value })}>
            <option value="quiet">Quiet — prefer silence</option>
